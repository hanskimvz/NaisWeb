...
foreach(($objmenu->config) as $grp => $obj){
	$leftMenu .= '<a class="nav-link collapsed" href="#" data-toggle="collapse" data-target="#'.$grp.'" aria-expanded="false" aria-controls="'.$grp.'"><span>'.$grp.'</span></a>';
    ...
    foreach($obj as $page => $param){
		...
		$leftMenu .= '<a id="'.($param->lang_key).'" class="collapse-item" href="'.$param->href.'"  target="contentFrame"><span>'.($param->lang_key).'</span></a>';
	}
	$leftMenu .= '</div></div>';
}
$leftMenu .= '</li>';
